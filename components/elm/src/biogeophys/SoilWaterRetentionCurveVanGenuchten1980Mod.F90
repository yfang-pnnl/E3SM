module SoilWaterRetentionCurveVanGenuchten1980Mod
  ...
  !---------------------------------------------------------------------------
  ! !DESCRIPTION:
  ! Implementation of soil_water_retention_curve_type using the VanGenuchten 1980
  ! parameterizations.
  !
  ! !USES:
  ...
  !
  ! !PUBLIC TYPES:
  public :: soil_water_retention_curve_van_genuchten_1980_type
  
  type, extends(soil_water_retention_curve_type) :: &
       soil_water_retention_curve_van_genuchten_1980_type
     private
   contains
  ...
  end type soil_water_retention_curve_van_genuchten_1980_type

  interface soil_water_retention_curve_van_genuchten_1980_type
     ! initialize a new soil_water_retention_curve_van_genuchten_1980_type object
     module procedure constructor  
  end interface soil_water_retention_curve_van_genuchten_1980_type

contains

  !-----------------------------------------------------------------------
  type(soil_water_retention_curve_van_genuchten_1980_type) function constructor()
    !
    ! !DESCRIPTION:
    ! Creates an object of type soil_water_retention_curve_van_genuchten_1980_type.
  ...
  subroutine soil_hk(this, hksat, imped, s, bsw, hk, dhkds)
    ...
    !
    ! !ARGUMENTS:
    class(soil_water_retention_curve_van_genuchten_1980_type), intent(in) :: this
    ...
    n = bsw
    m = 1.0  - 1.0/n
    hk=imped*hksat*sqrt(s)*(1.-(1.-s**(1./m))**m)**2
...
!place holder
  ...
  subroutine soil_suction(this, smpsat, s, bsw, smp, dsmpds)
    ...
    m = 1.-1./n
    smp = -(1._r8)
    smp = -(1._r8/alpha)*(s**(1._r8/(-m)) - 1._r8 )**(1/n)
    !compute derivative
