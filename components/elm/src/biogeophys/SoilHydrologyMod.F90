...
! Input:  [integer  (:)   ]  number of layers to bedrock                     
          ...
          qflx_rsub_sat_h3d  =>    col_wf%qflx_rsub_sat_h3d     , & ! Output: [real(r8) (:)   ] soil saturation excess from h3d [mm h2o/s]                 
          qflx_drain_h3d     =>    col_wf%qflx_drain_h3d        , & ! Output: [real(r8) (:)   ] sub-surface runoff from h3d(mm H2O /s)                    
...
!====================================================================================================================
   ...
   subroutine H3D_DRI(bounds, num_h3dc, filter_h3dc, jwt,zwt_h3d, tmp_rsub_top,h3d_rsub_top_max,fff,&
     ...
     use elm_time_manager , only : get_step_size
     use elm_varpar       , only : nlevsoi, nlevgrnd, nlayer, nlayert
     use elm_varpar       , only : nh3dc_per_lunit  !yhfang
     use elm_varcon       , only : pondmx, tfrz, watmin,rpi, secspday, nlvic
     use column_varcon    , only : icol_roof, icol_road_imperv, icol_road_perv
     use abortutils       , only : endrun
     use elm_varctl       , only : use_vsfm, use_var_soil_thick
