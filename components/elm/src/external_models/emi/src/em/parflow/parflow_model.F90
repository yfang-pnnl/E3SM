...
!   Initialize saturation
    if(.not. allocated(pf_sat) ) allocate(pf_sat(pf_num_nodes))
    if(.not. allocated(pf_press) ) allocate(pf_press(pf_num_nodes))
    if(.not. allocated(elm_flux) ) allocate(elm_flux(pf_num_nodes))
    if(.not. allocated(pf_por) ) allocate(pf_por(pf_num_nodes))
    elm_flux(1:pf_num_nodes) = 0.d0
    pf_sat(1:pf_num_nodes) = 0.d0
    pf_press(1:pf_num_nodes) = 0.d0
    pf_por(1:pf_num_nodes) = 0.d0
