...
#include <petsc/finclude/petscsys.h>
  ...
  type, public :: clm_parflow_idata_type
...
!  Vec :: thetares2_pf
  ...
  ! (i) Sink/Source of water for PARFLOW's 3D subsurface domain
  Vec :: qflx_clm   ! mpi vec
  Vec :: qflx_pf    ! seq vec
  
  ! (ii) Source of water and temperature of rain for PARFLOW's 2D surface domain
  Vec :: rain_clm   ! mpi vec
  Vec :: rain_pf    ! seq vec
  Vec :: rain_temp_clm ! mpi vec
  Vec :: rain_temp_pf  ! seq vec
  
  ! (iii) Ground heat flux BC for PARFLOW's subsurface domain
  !       This BC is applied on top surface of the subsurface domain
  Vec :: gflux_subsurf_clm  ! mpi vec
  Vec :: gflux_subsurf_pf   ! seq vec
  !       When running PARFLOW surface-subsurface simulation, ground heat flux
  !       is a SS for PARFLOW's surface domain.
  !
  ...
  PetscInt :: nlpf_sub  ! num of local parflow cells
  PetscInt :: ngpf_sub  ! num of ghosted parflow cells (ghosted = local+ghosts)
  ...
  PetscInt :: nlpf_2dsub   ! num of local parflow cells
  PetscInt :: ngpf_2dsub   ! num of ghosted parflow cells (ghosted = local+ghosts)
  ...
  PetscInt :: nlpf_srf   ! num of local parflow cells
  ...
  type(clm_parflow_idata_type) , public, target , save :: clm_pf_idata
  
  public :: CLMPARFLOWIDataInit, &
            CLMPARFLOWIDataCreateVec, &
            CLMPARFLOWIDataDestroy
  
contains

! ************************************************************************** !

  subroutine CLMPARFLOWIDataInit()
  ! 
...
! ************************************************************************** !
  ...
  subroutine CLMPARFLOWIDataCreateVec(mycomm)
  ! 
...
!    call MPI_Comm_rank(mycomm,rank, ierr)
    !
    ! For data transfer from CLM to PARFLOW
    !
    ...
    ! Create Seq. Vectors for PARFLOW
    ...
    !
    ! For data transfer from PARFLOW to CLM
    !

    ! Create MPI Vectors for PARFLOW
    ! 3D Subsurface PARFLOW ---to--- 3D Subsurface CLM
...
!    call VecDuplicate(clm_pf_idata%sat_clm,clm_pf_idata%thetares2_clm,ierr)
  ...
  end subroutine CLMPARFLOWIDataCreateVec

! ************************************************************************** !

  subroutine CLMPARFLOWIDataDestroy()
  ! 
...
!    if(clm_pf_idata%thetares2_pf      /= PETSC_NULL_VEC) call VecDestroy(clm_pf_idata%thetares2_pf,ierr)
  ...
  end subroutine CLMPARFLOWIDataDestroy

end module clm_parflow_interface_data
