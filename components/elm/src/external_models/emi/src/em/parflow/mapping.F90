...
!  integer, parameter,public :: MAXSTRINGLENGTH = 256 
!  integer, parameter, public :: MAXWORDLENGTH = 32
  private
  integer,parameter :: one_integer_mpi = 1
  type, public  :: mapping_type
    ...
    !
    ! In CLM-PARFLOW coupling, s and d vectors are decomposed over multiple processors.
    ...
    PetscInt           :: parflow_nlev             ! Number of PARFLOW layers
    PetscInt           :: parflow_nlev_mapped      ! Number of PARFLOW layers mapped
...
! ************************************************************************** !
  ...
  subroutine MappingReadTxtFile(map,map_filename,mycommsize,myrank)
  ! 
  ...
  ! 
    ...
    integer                         :: mycommsize
    integer                         :: fid
    integer, parameter              :: FIVE_INTEGER = 5
    ...
    if(myrank == io_rank) then
      fid = 20
      call InputCreate(fid,path,map_filename)
      nwts     = -1
      prev_row = -1
      ...
      do nheader = 1, 6
        ...
        call InputReadWord(string, card, .TRUE., ierr)
        call StringToLower(card)

        select case (trim(card))
          case('clm_nlevsoi')
            hint = 'clm_nlevsoi'
            call InputReadInt(string,map%clm_nlevsoi,ierr)
          case('clm_nlevgrnd')
            hint = 'clm_nlevgrnd'
            call InputReadInt(string,map%clm_nlevgrnd,ierr)
          case('clm_nlev_mapped')
            hint = 'clm_nlev_mapped'
            call InputReadInt(string,map%clm_nlev_mapped,ierr)
          case('parflow_nlev')
            hint = 'parflow_nlev'
            call InputReadInt(string,map%parflow_nlev,ierr)
          case('parflow_nlev_mapped')
            hint = 'parflow_nlev_mapped'
            call InputReadInt(string,map%parflow_nlev_mapped,ierr)
          case('num_weights')
            hint = 'num_weights'
            call InputReadInt(string,nwts,ierr)
            write(*,*),'nwts = ',nwts
          case default
            string = 'Unrecognized keyword "' // trim(card) // &
              '" in explicit grid file.'
            print *, 'ERROR: ' // trim(string)
        end select
      enddo
      
      nwts_tmp = nwts/mycommsize
      remainder= nwts - nwts_tmp*mycommsize
      
      allocate(wts_row_tmp(nwts_tmp + 1))
      allocate(wts_col_tmp(nwts_tmp + 1))
      allocate(wts_tmp(    nwts_tmp + 1))

      do irank = 0,mycommsize-1
        ...
        do ii = 1,nread
          call InputReadString(fid,string,ierr)
          call InputReadInt(string,wts_row_tmp(ii),ierr)
          call InputReadInt(string,wts_col_tmp(ii),ierr)
          call InputReadDouble(string,wts_tmp(ii),ierr)
        ...
        if (irank == myrank) then
          ...
          call MPI_Send(nread,1,MPI_INTEGER,irank,myrank,MPI_COMM_WORLD, &
                        ierr)
          call MPI_Send(wts_row_tmp,nread,MPI_INTEGER,irank,myrank, &
                        MPI_COMM_WORLD,ierr)
          call MPI_Send(wts_col_tmp,nread,MPI_INTEGER,irank,myrank, &
                        MPI_COMM_WORLD,ierr)
          call MPI_Send(wts_tmp,nread,MPI_DOUBLE_PRECISION,irank,myrank, &
                        MPI_COMM_WORLD,ierr)
      ...
      call MPI_Recv(map%s2d_nwts,1,MPI_INTEGER,0,MPI_ANY_TAG, &
      ...
      call MPI_Recv(map%s2d_icsr,map%s2d_nwts,MPI_INTEGER,0,MPI_ANY_TAG, &
                    MPI_COMM_WORLD,status_mpi,ierr)
      call MPI_Recv(map%s2d_jcsr,map%s2d_nwts,MPI_INTEGER,0,MPI_ANY_TAG, &
                    MPI_COMM_WORLD,status_mpi,ierr)
      call MPI_Recv(map%s2d_wts,map%s2d_nwts,MPI_DOUBLE_PRECISION,0,MPI_ANY_TAG, &
                    MPI_COMM_WORLD,status_mpi,ierr)
                    
    endif

  ! Broadcast from root information regarding CLM/PARFLOW num soil layers
  temp_int_array(1) = map%clm_nlevsoi
  temp_int_array(2) = map%clm_nlevgrnd
  temp_int_array(3) = map%clm_nlev_mapped
  temp_int_array(4) = map%parflow_nlev
  temp_int_array(5) = map%parflow_nlev_mapped

  call MPI_Bcast(temp_int_array,FIVE_INTEGER,MPI_INTEGER,0, &
                 MPI_COMM_WORLD,ierr)
    
  map%clm_nlevsoi = temp_int_array(1)
  map%clm_nlevgrnd = temp_int_array(2)
  map%clm_nlev_mapped = temp_int_array(3)
  map%parflow_nlev = temp_int_array(4)
  map%parflow_nlev_mapped = temp_int_array(5)

  end subroutine MappingReadTxtFile

! ************************************************************************** !

  subroutine MappingReadHDF5(map,map_filename,mycommsize,myrank)
  ! 
...
#ifndef SERIAL_HDF5
    call h5pset_fapl_mpio_f(prop_id,MPI_COMM_WORLD,MPI_INFO_NULL,hdf5_err)
#endif
...
!    endif
    ...
    map%s2d_nwts=INT(dims_h5(1))/mycommsize
    remainder=INT(dims_h5(1))-map%s2d_nwts*mycommsize
    if (myrank < remainder) map%s2d_nwts=map%s2d_nwts + 1
    ...
    call MPI_Exscan(map%s2d_nwts,istart,ONE_INTEGER_MPI, &
                    MPIU_INTEGER,MPI_SUM,MPI_COMM_WORLD,ierr)
