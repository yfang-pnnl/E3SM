...
additional details. In ELM, $f_{over}$ is the grid-cell parameter $f_{drain}$ ($m^{-1}$)
...
The recharge rate to the unconfined aquifer, $q_{charge}$
($mm\ s^{-1}$, positive into the aquifer), is computed as a Darcy flux
between the unsaturated soil node just above the water table and the
water table itself

$$q_{charge} = k_a \frac{\psi_m\!\left(z_{jwt}\right) - \psi_{eq}\!\left(z_{jwt}\right)}
               {2\left(z_\nabla - z_{jwt}\right)\times 10^3}$$

where $k_a$ ($mm\ s^{-1}$) is the unsaturated hydraulic conductivity at
the first layer above the water table (layer index $jwt + 1$)

$$k_a = \Theta_{ice} \, k_{sat,\,jwt+1}
        \left(\frac{\theta_{jwt+1}}{\theta_{sat,\,jwt+1}}\right)^{2B_{jwt+1}+3},$$

$z_{jwt}$ (m) is the node depth of the layer just above the water table,
$z_\nabla$ (m) is the water table depth, $\psi_m(z_{jwt})$ (mm) is the
soil matric potential at that node, $\psi_{eq}(z_{jwt})$ (mm) is the
equilibrium matric potential at the same node for the current water table
position, and $10^3$ converts depth from m to mm. The factor of 2
approximates the flux at the midpoint between the node and the water
table.

When the water table is below the soil column, the recharge is diagnosed
directly from the change in moisture of the notional aquifer layer
produced by the tridiagonal solve. The recharge updates the water stored
in the unconfined aquifer $W_a$ (mm)

$$W_a^{n+1} = W_a^{n} + q_{charge} \, \Delta t .$$

When the water table is within the soil column, $q_{charge}$ moves the
water table through the specific yield $S_y$ (section
`Lateral Sub-surface Runoff`{.interpreted-text role="numref"}).

## Lateral Sub-surface Runoff

Lateral sub-surface runoff follows a TOPMODEL-based exponential decay
with water table depth (`Niu et al. 2005 <Niuetal2005>`{.interpreted-text
role="ref"}). The sub-surface drainage flux ($mm\ s^{-1}$) is

$$q_{drai} = \Theta_{ice} \cdot K_{drai,max} \cdot \exp\!\left(-f\,z_\nabla\right) ,$$

where $\Theta_{ice}$ is a thickness-weighted ice impedance factor
(section `Hydraulic Properties`{.interpreted-text role="numref"}),
$K_{drai,max}$ ($mm\ s^{-1}$) is the maximum drainage rate, $f$
($m^{-1}$) is a decay factor, and $z_\nabla$ (m) is the water table
depth.

The maximum drainage rate is limited by topographic slope and a global
upper bound

$$K_{drai,max} = \min\!\left(10\sin\beta,\, K_{drai,max,global}\right) ,$$

where $\beta$ is the mean grid-cell topographic slope (radians) and
$K_{drai,max,global}$ is a global upper limit (default $10\ mm\ s^{-1}$).
The decay factor is $f = f_{drain}$ ($m^{-1}$), a grid-cell parameter
read from the surface dataset (fallback default $2.5\ m^{-1}$).

The ice impedance factor is a thickness-weighted average over layers
from the water-table layer ($j_\nabla$) to the bedrock layer
($N_{bed}$)

$$\Theta_{ice} = 10^{-\Omega\bar{F}_{ice}}, \qquad
\bar{F}_{ice} =
\frac{\displaystyle\sum_{i=j_\nabla}^{N_{bed}} f_{ice,i}\,\Delta z_i}
     {\displaystyle\sum_{i=j_\nabla}^{N_{bed}} \Delta z_i}$$

where $\Omega = 6$, $f_{ice,i} = \theta_{ice,i}/\theta_{sat,i}$ is the
ice-filled pore fraction of layer $i$, and $\Delta z_i$ (mm) is the
layer thickness.

The water table depth $z_\nabla$ is determined by finding the deepest
soil layer above the bedrock depth (section
`Depth to Bedrock`{.interpreted-text role="numref"}) in which the
volumetric water content falls below a threshold of 0.9.
