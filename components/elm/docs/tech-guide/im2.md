...
3. **Distributes received water to columns.** Each timestep, a fixed
   ...
   input at the top of the soil profile ($q_{\text{top\_soil}}$).
...
$$
\Delta S_{t'} = q_{\text{to\_downhill}}(c)\;\Delta t
...
| $q_{\text{to\_downhill}}$ | `col_wf%qflx_to_downhill(c)` | mm s⁻¹ | Column flux sent to downhill topounit |
| $q_{\text{from\_uphill}}$ | `col_wf%qflx_from_uphill(c)` | mm s⁻¹ | Column flux received from uphill topounit |
