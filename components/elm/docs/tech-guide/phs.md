...
from stem to shaded leaf ($q_{1a}$ and $q_{1b}$). The water flux from
...
$\psi_{stem}$, $\psi_{shadeleaf}$, $\psi_{sunleaf}$). The fluxes
...
q_{soil}&=\sum_{i=1}^{nlevsoi}{q_{3,i}}=\sum_{i=1}^{nlevsoi}{k_{3,i}\cdot\left(\psi_{soil,i}-\psi_{root} + \Delta\psi_{z,i} \right)}
\end{aligned}
\end{aligned}$$
...
$\psi_{stem}$, $\psi_{shadeleaf}$, $\psi_{sunleaf}$) satisfying
